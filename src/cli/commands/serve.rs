...
fn execute_mcp_tool(
    ...
    let output = match name {
        ...
        .execute(input)?,
        "rlm_process_wait" => RlmLiveWaitTool {
            config: state.config.clone(),
        }
        .execute(input)?,
        "rlm_process_cancel" => {
        ...
        "rlm_process_recover" => {
            if !mcp_write_tools_enabled(state) {
                return Err(app_error(
                    "MCP RLM live recovery tool `rlm_process_recover` is disabled; set DSCODE_MCP_ENABLE_DURABLE_APPROVALS=1 or DSCODE_MCP_APPROVAL_THREAD_ID=<thread-id> to route runtime-state writes through approvals",
                ));
            }
            let target = format!(
                "recover live RLM session: {}",
                input
                    .get("session_id")
                    .map(str::trim)
                    .filter(|value| !value.is_empty())
            ...
            request_mcp_runtime_write_approval(state, &input, "rlm_process_recover", target)?;
            RlmLiveRecoverTool {
                config: state.config.clone(),
            }
            .execute(input)?
        ...
        "rlm_process_stop" => {
            if !mcp_write_tools_enabled(state) {
                return Err(app_error(
                    "MCP RLM live stop tool `rlm_process_stop` is disabled; set DSCODE_MCP_ENABLE_DURABLE_APPROVALS=1 or DSCODE_MCP_APPROVAL_THREAD_ID=<thread-id> to route runtime-state writes through approvals",
                ));
            }
            let target = format!(
                "stop live RLM session: {}",
                input
                    .get("session_id")
                    .map(str::trim)
                    .filter(|value| !value.is_empty())
            ...
            request_mcp_runtime_write_approval(state, &input, "rlm_process_stop", target)?;
            RlmLiveStopTool {
                config: state.config.clone(),
            }
            .execute(input)?
...
fn mcp_tool_definitions(state: &McpStdioState) -> Vec<JsonValue> {
        ...
        mcp_tool_definition(
            ...
            mcp_schema(
                vec![
                    ...
                    ("cursor", number_property("Return events with seq greater than this cursor.")),
                    ("limit", number_property("Maximum events to return.")),
                    ("timeout_ms", number_property("Maximum wait in milliseconds.")),
                    ("poll_interval_ms", number_property("Polling interval in milliseconds.")),
                ],
                &["session_id"],
            ),
        ),
    ...
    if mcp_write_tools_enabled(state) {
        ...
        tools.push(mcp_tool_definition(
            "rlm_process_recover",
            "Recover interrupted live RLM daemon turns by requeueing or failing stale running turns, skipping turns owned by a live daemon pid unless force=true. Requires durable runtime approvals.",
            mcp_schema(
                vec![
                    ...
                    ("force", string_property("Set true to recover even when daemon_pid is still alive.")),
        ...
        tools.push(mcp_tool_definition(
            "rlm_process_stop",
            "Stop an idle live RLM daemon session, canceling queued turns and writing a session_stopped event. Requires durable runtime approvals.",
            mcp_schema(
                vec![
                    ("session_id", string_property("Live RLM session id.")),
                    ("reason", string_property("Optional stop reason.")),
                ],
                &["session_id"],
            ),
        ));
...
mod tests {
    ...
    fn mcp_tools_list_includes_workspace_and_runtime_tools() {
        ...
        assert!(!rendered.contains(r#""name":"rlm_process_recover""#));
        assert!(!rendered.contains(r#""name":"rlm_process_stop""#));
