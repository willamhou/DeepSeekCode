...
fn live_evidence_verification_status(path: Option<&str>) -> PublishStatusCheck {
    ...
    if json_field_bool(&root, "mcp_dynamic_surface_case_present") != Some(true) {
        failures.push("verification is missing MCP dynamic loop-surface evidence".to_string());
    }
    if json_field_bool(&root, "mcp_resource_surface_case_present") != Some(true) {
        failures.push("verification is missing MCP resource loop-surface evidence".to_string());
    }
...
mod tests {
    ...
    #[test]
    fn publish_status_blocks_live_evidence_without_mcp_resource_surface() {
        let root = temp_root("publish-live-evidence-resource-surface-blocked");
        std::fs::create_dir_all(&root).unwrap();
        let live_evidence = root.join("live-evidence-verification.json");
        std::fs::write(
            &live_evidence,
        ...
        assert_eq!(check.status, PublishStatus::Blocked);
        assert!(check.detail.contains("MCP resource loop-surface evidence"));
    }
