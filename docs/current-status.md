...
- 本轮新增：在线 DeepSeek dogfood smoke 第一片。使用仓库外 key 文件验证 `dogfood live-run --api-key-file ... --execute` 进入 `model_transport=online`：`write_validate` 1 条成功，`recovery` 1 条成功，`pr_workflow` 首条 GitHub Action `@deepseek fix` case 暴露了真实模型重复 `project_map` 后 stuck 的问题；随后 runtime 增加 direct-edit guardrail，当任务明确要求 `replace X with Y in path` 且模型已读到目标内容时，会在 remote 模式下强制执行 `apply_patch`、patch 后强制运行建议测试、测试通过后强制 finish。继续跑多个小批次后，retry case 已验证能从 `a * b` 测试失败自动回退到 `a + b` 并通过 `cargo test`；live-plan 也会跳过预期失败读回样本，不再把它当作 release live 成功样本。最新一批真实 `pr_workflow` 复跑又暴露两个 reproduce-and-fix case 在只做 `project_map`/readback 后没有及时 patch；runtime 现在对显式 `replace X with Y in path` 任务只要已有 repo 上下文即可触发 guardrail patch，再跑建议测试并在通过后 finish。复跑同一 5-case `pr_workflow` live batch 全部成功，之前卡住的 Python/Rust reproduce-and-fix 分别完成 `src/app.py`、`src/cli/app.rs` 替换并通过 `pytest`/`cargo test`。当前本机 live ledger 为 `30` 条 online run、`26` success、`4` historical stuck；最新小门槛 `live-runs=30`、overall success `85%`、`write_validate:5:90`、`recovery:10:90`、`pr_workflow:15:80` 已通过，但完整 release gate 仍失败：还需要 `100` 条 online run，三类各 `25` 条，并且 overall 与各类成功率都要 `>=90%`。
...
4. 产品打磨
   ...
   - GitHub automation 已有 event bridge、review/fix/patch mode routing、review workflow 示例、写入型 PR-head checkout workflow 示例、本地 workflow fixture smoke、`25` 条 `pr_workflow` benchmark cases，以及 `14/14` offline `pr_workflow` dogfood replay；当前本机已有 `15` 条 online model-backed `pr_workflow` 样本，其中最近一批 direct-edit guardrail 修复后 `5/5` 通过，但累计仍为 `12/15`，还缺真实远端 fixture repo 上的 workflow run 证据和足够多 online `pr_workflow` 样本把累计成功率拉回 `>=90%`。
   ...
   - Hooks 已有 prompt submit、session start/stop、pre/post tool use 和 structured allow/add_context 的 `deepseek hooks fixture-smoke --json` 本地 gate；skills/custom command 已有 `deepseek skills validate --strict --json` 元数据 gate 和 discovery 文档；subagent 已有 `deepseek agents subagent-fixture-smoke --json` gate、20 条 subagent benchmark cases、targeted subagent benchmark `20/20`、完整默认 benchmark `82/82`、并发 child readback 和 write-scope conflict metadata。Phase 12D 本地 extension gates 已基本齐备，offline dogfood coverage gate 也已补到 `runs=20` 并通过；下一步应继续真实 model-backed dogfood 和外部兼容性证据。
...
2. 跑真实 model-backed dogfood
   - 先轮换任何已经泄漏到聊天记录里的 key。
   - 用仓库外 key 文件，例如 `/tmp/deepseek-demo.key`，执行 model-backed demo recorder；live dogfood 可用 `deepseek dogfood live-run --api-key-file /tmp/deepseek-demo.key --limit 3 --execute`。
