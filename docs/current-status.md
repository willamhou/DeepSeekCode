...
- 本轮新增：在线 DeepSeek dogfood smoke 第一片。使用仓库外 key 文件验证 `dogfood live-run --api-key-file ... --execute` 进入 `model_transport=online`：`write_validate` 1 条成功，`recovery` 1 条成功，`pr_workflow` 首条 GitHub Action `@deepseek fix` case 暴露了真实模型重复 `project_map` 后 stuck 的问题；随后 runtime 增加 direct-edit guardrail，当任务明确要求 `replace X with Y in path` 且模型已读到目标内容时，会在 remote 模式下强制执行 `apply_patch`、patch 后强制运行建议测试、测试通过后强制 finish。复跑同一 `pr_workflow` case 后成功完成 `src/index.js` 替换并通过 `npm test`。继续跑多个小批次后，retry case 已验证能从 `a * b` 测试失败自动回退到 `a + b` 并通过 `cargo test`；live-plan 也会跳过预期失败读回样本，不再把它当作 release live 成功样本。当前本机 live ledger 为 `20` 条 online run、`18` success、`2` historical stuck；最新小门槛 `live-runs=20`、overall success `90%`、`write_validate:5:90`、`recovery:10:90`、`pr_workflow:5:80` 已通过，但完整 release gate 仍要求 `100` 条 online run 和三类各 `25` 条且成功率 `>=90%`。
...
4. 产品打磨
   ...
   - GitHub automation 已有 event bridge、review/fix/patch mode routing、review workflow 示例、写入型 PR-head checkout workflow 示例、本地 workflow fixture smoke、`25` 条 `pr_workflow` benchmark cases，以及 `14/14` offline `pr_workflow` dogfood replay；当前还缺真实远端 fixture repo 上的 workflow run 证据和 online model-backed `pr_workflow` dogfood 样本。
