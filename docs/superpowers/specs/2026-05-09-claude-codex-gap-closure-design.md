...
4. IDE / 编辑器配套仍只有最小入口，MCP/plugin 生态已有配置发现、stdio/HTTP `tools/list`、manual `tools/call`、generic agent bridge 和 bridge 级审批/allowlist，但动态独立 tool 注入、旧式 SSE 和更完整 permission UX 仍缺失，云端/外部任务面仍缺失
