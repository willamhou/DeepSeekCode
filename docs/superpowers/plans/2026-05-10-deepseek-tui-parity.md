...
-> `replay` against a small shell command, verifies Linux `tty=true` jobs use
the `native-supervisor` PTY backend, and additionally proves PTY `stdin`,
`resize`, terminal `replay`, and `cancel` before requesting `shutdown` and
emitting local JSON evidence without requiring systemd/launchd installation; it
also blocks too-long Unix socket paths before spawning the supervisor and points
release smoke runs at short isolated workdirs such as `/tmp/dsc-smk`.
...
checks now run the recorder dry-run, redaction self-test, verifier self-test,
...
- TUI now supports DeepSeek-TUI-style `/goal`, tracking an active-thread
  runtime-backed objective with optional token budget progress from
  active-thread usage telemetry; file-backed and HTTP-runtime sessions restore
  it from durable goal events after restart or reconnect
...
- `deepseek agents service-smoke` starts `serve --http --once` from the selected
  ...
  `replay`, verifies Linux `tty=true` jobs use the `native-supervisor` PTY
  backend, proves PTY `stdin`/`resize`/terminal `replay`/`cancel`, blocks
