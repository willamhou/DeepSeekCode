...
- Prompt-layer hash diagnostics, `deepseek stats`, and `deepseek events
  ...
  cache-stable prompt-layer hash regressions, and daemon compaction threshold /
  keep-tail policy can be tuned through runtime config. The release matrix now
  uploads deterministic repair/cache and prompt-prefix-stability JSON evidence
