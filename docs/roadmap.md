...
- Model presets and first-pass budget controls have landed:
  ...
  seven-day live recency gate; the first real online MCP surface batch passed
  locally with `mcp 27/30`, and stale-recency refreshes are now surfaced by
  `deepseek dogfood live-plan`. The remaining work is recurring live dogfood
  calibration with real online runs.
