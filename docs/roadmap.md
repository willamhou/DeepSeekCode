...
- verified online multi-file external fixture evidence is tracked for the
  ...
  `.dscode/dogfood/`.
- `v0.1.6` ships GitHub Release assets for Linux x64, Linux arm64, macOS x64,
...
- `v0.1.6` public release archives are verified on hosted Linux x64, Linux
  arm64, macOS x64, and macOS arm64 by Release Smoke:
  https://github.com/willamhou/DeepSeekCode/actions/runs/26803043308
- `v0.1.6` npm/npx packages are published and verified through registry lookup,
  `npx @deepseek-code/cli@0.1.6 version`, and clean-directory
...
- Homebrew is published and verified for `v0.1.6`; keep the tap formula aligned
...
- `v0.1.6` platform npm packages and the root wrapper are published and
...
- Model presets and first-pass budget controls have landed:
  ...
  seven-day live recency gate; the latest real online MCP surface batch passed
  locally on 2026-06-01 with `mcp 31/34`, and stale-recency refreshes are
  surfaced by `deepseek dogfood live-plan`. The remaining work is recurring
  live dogfood calibration with real online runs.
